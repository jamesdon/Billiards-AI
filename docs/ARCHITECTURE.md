...
First-time bring-up uses the FastAPI-mounted **setup guide** at **`GET /setup`** (default **`http://127.0.0.1:8000/setup`**). Start the API with **`scripts/run_backend.sh`**, then follow checklists and doc links in the UI. Step titles match the table in **`docs/TEST_PLAN.md`** and the numbered runbooks **`docs/1` … `docs/9`**. Details: **`README.md`**, **`docs/PORTS.md`**, **`backend/setup_guide.py`**, **`backend/static/setup/`**.
