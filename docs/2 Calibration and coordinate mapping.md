...
**Jetson CSI before the GUI:** if `nvarguscamerasrc` prints **No cameras available**, Argus does not see any CSI sensor — re-seat the ribbon, restart **`nvargus-daemon`**, run **`bash scripts/jetson_csi_setup.sh`**, then **`start_calibration.sh`** again. **`CSI_SENSOR_ID`** is the **Argus camera index** (0, 1, …) for sensors that **successfully probed**, not a guaranteed mapping to physical CAM0 vs CAM1. If the second module never probes (e.g. `imx477` **i2c read probe (-121)** in `dmesg`), Argus may only expose **index 0** — **`CSI_SENSOR_ID=1`** will then fail with **Invalid camera device … 0 max index**. `start_calibration.sh` runs a short **CSI preflight** on Linux aarch64 so the GUI does not start until Argus can grab one frame (override with **`SKIP_CSI_PREFLIGHT=1`** only if you must).
