...
Billiards-AI/
  docs/
    ...
    4 Classification and identity.md  Optional **§4** (profiles + ball classes); same wizard step as `docs/3` §3; `/api/setup/profiles-*`
  ...
  core/                             Shared logic (no OpenCV/YOLO dependency)
    ...
    identities_path.py              Repo-root `identities.json` path (API + edge default `--identities`)
  ...
  backend/                          Optional offload + history + dashboard API
    ...
    setup_guide.py                  Guided setup: `GET /api/setup/steps` (step titles in `docs/TEST_PLAN.md`); **`phase3`** = **Detection, tracking, classification, and identity** (single step replacing former detection + classification steps): **five** checklist lines (one **edge.main** + MJPEG+health, **ball** labels, **non-empty** profiles, **display_name**, optional restart; **Live profile status** `GET /api/setup/profiles-status`, bootstrap, profile links), file-backed `identities.json`, `GET/PUT /api/setup/progress`, `GET /api/setup/context` (includes `api_port`, `api_default_port` 8000, `mjpeg_default_port` 8001, `public_http_base`, `public_http_base_source`, `scorekeeper_url` from `backend/lan_url.py` for QR/links; see `docs/PORTS.md`), `GET /api/setup/edge-health?port=` (server probes `http://127.0.0.1:{port}/health` for `edge.main` MJPEG; response includes `reason: connection_refused` when nothing listens, since the browser cannot read that cross-origin from the guide HTTP port without CORS), `GET /api/setup/doc?path=&textSize=` (Markdown→HTML; `textSize` read case-insensitively from query; first-paint size from query, then `setup_text_size` cookie, then `medium`; all viewer `href` values entity-encode `&` as `&amp;` so Safari does not truncate URLs; root font from `html[data-text-size]` + `!important`; `Set-Cookie` on each doc view), `GET /scorekeeper` (serves `static/scorekeeper/index.html`), optional `POST /api/setup/launch` when `SETUP_ALLOW_LAUNCH=1`
    lan_url.py                      `public_http_base` / `scorekeeper_url` for LAN-friendly links (request Host, else guessed IPv4, else loopback)
    static/setup/                   Setup wizard UI (`index.html`: sidebar stacks **API + MJPEG port status** first, then **Setup guide** vs **Score Keeper** tabs, `#sk-qr-block`, `#nav-steps`, footer—so port status does not move when switching mode/QR; `body[data-ui-mode]`; `main#content` + `#sk-embed` iframe in `.main-wrap` (**setup:** `#sk-embed` `inert` + `body[data-ui-mode=setup]` CSS so the hidden Score Keeper layer cannot intercept pointer events); `app.js`, `style.css`; in Score Keeper mode `#nav-steps` is hidden; `setup_text_size` cookie + `localStorage` `billiards-setup-text-size` and `billiards-setup-ui-mode`; **Traffic lights** (`.health-lamp` green/gray/red): `GET /health` for API port (`api_port` in sidebar) every 10s, and for MJPEG port **edge** + **stream** via `GET /api/setup/edge-health?port=` (server probes `127.0.0.1:{port}/health`); MJPEG field 8001–8005; `href_template` may include `{api_port}` and `{mjpeg_port}`; checklist `verify_actions` as `<button>` elements that `window.open` MJPEG/health URLs; **Copy** on fenced ` ``` ` command blocks in **How to verify** only (inline single backticks = styling, no Copy; step **summary** and **Tips** use the same rich renderer); `setup_guide` strings use ` ```…``` ` for paste-ready shell; doc links add `&textSize=`; progress in `localStorage` and `data/setup_wizard_progress.json`; resizable sidebar; `pagehide`/`beforeunload` keepalive save
    static/scorekeeper/             Standalone **Score Keeper** UI (`index.html`, `app.js`, `style.css`): same `data-text-size` + `localStorage` key `billiards-setup-text-size` as the setup guide; `postMessage` + `storage` to sync from the parent iframe; polls `GET /live/state`, WebSocket to same host `/ws` for live game view; **Player & stick names** block loads `GET /profiles`, `PATCH` per row (`/profiles/player/{id}` / `/profiles/stick/{id}`) for `identities.json` display names
  ...
  scripts/
    ...
    jetson_mic_peak_check.py        Stdlib WAV peak/RMS for headless USB mic checks (no `aplay`)
    test_venue_usb_mics_headless.sh  `arecord` each `MIC_DEVICES` + peak/RMS; env: `RECORD_SECONDS`, `SAMPLE_RATE`, `MIN_PEAK`, `OUT_DIR` (see `docs/HARDWARE_IMX477_AUDIO.md`)
  ...
  requirements-audio.txt            Optional `sounddevice` for `--mic-device`; OS needs `libportaudio2` (see file header + `docs/HARDWARE_IMX477_AUDIO.md`)
