...
Billiards-AI/
  ...
  backend/                          Optional offload + history + dashboard API
    ...
    setup_guide.py                  Guided setup: `GET /api/setup/steps`, `GET/PUT /api/setup/progress`, `GET /api/setup/doc?path=&textSize=` (Markdown→HTML; `textSize=small|medium|large` syncs with setup UI), optional `POST /api/setup/launch` when `SETUP_ALLOW_LAUNCH=1`
    static/setup/                   Setup wizard UI (`index.html`, `app.js`, `style.css`); doc links to `/api/setup/doc?…&textSize=` match the current size (falls back to `localStorage`); checklist Copy is only for **How to verify** backticks that look like shell, not “What to record”; command blocks use `data-copy` for the exact string; text size in `localStorage` (`billiards-setup-text-size`); progress in `localStorage` `billiards-setup-progress-v1` and `data/setup_wizard_progress.json`; `pagehide`/`beforeunload` keepalive save; resizable sidebar (`#sidebar-resize`, `localStorage` `billiards-setup-sidebar-width-px`, hidden on narrow viewports)
