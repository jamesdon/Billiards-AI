...
Billiards-AI/
  ...
  backend/                          Optional offload + history + dashboard API
    ...
    setup_guide.py                  Guided setup: `GET /api/setup/steps`, `GET/PUT /api/setup/progress`, `GET /api/setup/doc?path=&textSize=` (Markdown→HTML; first-paint `textSize` from query, then `setup_text_size` cookie, then `medium`; all `/api/setup/doc` hrefs in the article are rewritten to include `textSize`; `Set-Cookie` on each doc view), optional `POST /api/setup/launch` when `SETUP_ALLOW_LAUNCH=1`
    static/setup/                   Setup wizard UI (`index.html`, `app.js`, `style.css`); `setup_text_size` cookie + `localStorage` `billiards-setup-text-size` mirror; doc links add `&textSize=`; optional per-checklist `record_paste` + **Copy for Notes (template)**; verify line Copy = shell; command blocks use `data-copy`; progress in `localStorage` `billiards-setup-progress-v1` and `data/setup_wizard_progress.json`; resizable sidebar (`#sidebar-resize`); `pagehide`/`beforeunload` keepalive save
