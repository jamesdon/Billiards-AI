...
Billiards-AI/
  ...
  backend/                          Optional offload + history + dashboard API
    ...
    static/scorekeeper/             Standalone **Score Keeper** UI (`index.html`, `app.js`, `style.css`): same `data-text-size` + `localStorage` key `billiards-setup-text-size` as the setup guide; `postMessage` + `storage` to sync from the parent iframe; polls `GET /live/state`, WebSocket to same host `/ws` for live game view (no table overlay)
