...
Billiards-AI/
  ...
  backend/                          Optional offload + history + dashboard API
    ...
    static/setup/                   Setup wizard UI (`index.html`, `app.js`, `style.css`); `setup_text_size` cookie + `localStorage` `billiards-setup-text-size`; **Traffic lights** (`.health-lamp` green/gray/red): `GET /health` for API port (`api_port` in sidebar) every 10s, and for MJPEG port **edge** + **stream** via `GET /api/setup/edge-health?port=` (server probes `127.0.0.1:{port}/health`); MJPEG field 8001–8005; `href_template` may include `{api_port}` and `{mjpeg_port}`; checklist `verify_actions` as `<button>` elements that `window.open` MJPEG/health URLs; **Copy** on checklist **How to verify** backtick one-liners only (Tips use backticks for code style, no Copy); doc links add `&textSize=`; progress in `localStorage` and `data/setup_wizard_progress.json`; resizable sidebar; `pagehide`/`beforeunload` keepalive save
