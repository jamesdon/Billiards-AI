...
Billiards-AI/
  ...
  scripts/
    ...
    calib_click.py                  Interactive calibration: physical TL/TR (kitchen short rail) and BL/BR (foot), auto outside corners; live CSI/USB background each frame (still image with `--frame`); draggable control panel with non-overlapping sections (table list, selected size, units, view controls), fullscreen maximize, writes calibration.json; side-pocket pixel UI removed (JSON still has LS/RS from homography defaults); CLI camera via start_calibration.sh
