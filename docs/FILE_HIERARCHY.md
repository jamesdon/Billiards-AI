...
Billiards-AI/
  ...
  backend/                          Optional offload + history + dashboard API
    ...
    static/scorekeeper/             Standalone **Score Keeper** UI (`index.html`, `app.js`, `style.css`): polls `GET /live/state`, WebSocket to same host `/ws` for live game view (no table overlay)
