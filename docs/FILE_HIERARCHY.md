...
Billiards-AI/
  ...
  backend/                          Optional offload + history + dashboard API
    ...
    static/setup/                   Setup wizard UI (`index.html`, `app.js`, `style.css`); `setup_text_size` cookie + `localStorage` `billiards-setup-text-size`; sidebar API base URL + MJPEG port + `GET /api/setup/edge-health?port=` (server probes `http://127.0.0.1:{port}/health` only, not a `scripts/phase3.sh` result); `href_template` may include `{api_port}` and `{mjpeg_port}`; checklist `verify_actions` as `<button>` elements that `window.open` MJPEG/health URLs; **Copy** for full shell one-liner backticks (e.g. `tail` log); doc links add `&textSize=`; progress in `localStorage` and `data/setup_wizard_progress.json`; resizable sidebar; `pagehide`/`beforeunload` keepalive save
