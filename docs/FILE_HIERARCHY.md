...
Billiards-AI/
  ...
  backend/                          Optional offload + history + dashboard API
    ...
    static/setup/                   Setup wizard UI (`index.html`, `app.js`, `style.css`) served under `/static/setup/`; text size radios in sidebar footer + `localStorage` key `billiards-setup-text-size`; root `font-size` set in px via inline script + `app.js`
