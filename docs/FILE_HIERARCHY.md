...
Billiards-AI/
  ...
  scripts/
    ...
    calib_click.py                  Interactive calibration: TL/TR/BL/BR at corner-pocket **inner throat**; inserts repo root on `sys.path` first so `import edge` works when run as `python scripts/calib_click.py` (not only as a module). Overlay: kitchen/foot tints, `table_diagram_m` (grid, break box, long/transverse/foot strings, head string, side pockets, rail diamonds, 2-1/4" rack at foot, captioned labels), then playfield polyline. `_estimate_outside_corners` avoids picking the **largest** hull quad (often the room outline) by scoring quads in an area band and with border inset; `_order_physical_table_corners` tries both kitchen-at-top vs kitchen-at-bottom hypotheses vs image-axis order; Hough + `_pocket_throat_from_seed`; `warpAffine` + `BORDER_CONSTANT` for pan/zoom voids; writes calibration.json; CLI via start_calibration.sh
