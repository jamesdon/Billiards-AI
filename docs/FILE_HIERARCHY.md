...
Billiards-AI/
  ...
  scripts/
    ...
    calib_click.py                  Interactive calibration: TL/TR/BL/BR at corner-pocket **inner throat** (extended rail-line intersection); auto seeds from frame then drag; live CSI/USB only; two-column SETUP/VIEW panel (reserved label bands, symmetric pan tiles, primary reset); writes calibration.json; side-pocket GUI removed (JSON LS/RS from defaults); CLI via start_calibration.sh
